...
if second_language == 'all':
    ...
    for language in language_list:
        ...
        if len(lst_words) == 0 and len(lst_phrases) == 0:
            ...
            print(lst_words[0])
            file.write(f'\n{second_language.capitalize()} Translations:\n')
            file.write(f'{lst_words[0]}\n')

            print(f'\n{second_language.capitalize()} Examples:')
            print(lst_phrases[0])
            print(lst_phrases[1])
            file.write(f'\n{second_language.capitalize()} Examples:\n')
            file.write(f'{lst_phrases[0]}\n')
            file.write(f'{lst_phrases[1]}\n')
    ...
    if second_language.capitalize() not in language_list:
        ...
        if len(lst_words) == 0 and len(lst_phrases) == 0:
            ...
            file.write(f'{second_language.capitalize()} Translations:\n')
            for index in range(5):
                print(lst_words[index])
                file.write(f'{lst_words[index]}\n')
            print(f'\n{second_language.capitalize()} Examples:')
            file.write(f'\n{second_language.capitalize()} Examples:\n')
            for index in range(0, 10, 2):
                print(lst_phrases[index])
                print(lst_phrases[index + 1], '\n')
                file.write(f'{lst_phrases[index]}\n')
                file.write(f'{lst_phrases[index + 1]}\n\n')
